 ...
 *
 * 單設備故障隔離（mutex 之外的硬體層防護）：
 *   mutex 只防「同時拉 CS」的軟體競用，擋不住故障從設備在 CS=HIGH 時仍持續
 *   驅動共用 MISO。SX126x 系（E80）於 NRST 拉低時全腳高阻，故 E80 停用或偵測
 *   失敗時一律由 LoRaE80_Shutdown() 把 RST 拉低，使其自匯流排物理斷開，
 *   保護 Flash 飛行記錄不被污染。Flash 端 CS 預設拉高，本身即隔離。
 ******************************************************************************
 */
#ifndef __SPI3_BUS_H
#define __SPI3_BUS_H
