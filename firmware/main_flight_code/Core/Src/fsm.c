...
FSM_Action_t FSM_Step(FSM_Context_t *ctx, const FSM_Input_t *in)
{
    ...
    switch (ctx->state) {
        ...
        case STATE_COAST: {
            ...
            // 4. P0-B：baro 原始趨勢交叉檢查（上方 baro_apogee，不依賴 EKF，恆生效）
            // 同時 1~3 必須滿足起飛時間鎖（起飛後累計大於 3.0 秒）
            // P0-C：EKF unhealthy 時停用 1~3（發散的 h_est/v_est 會誤點火），僅留 4 + 失效計時器
        ...
        case STATE_DEPLOY_DROGUE:
            // 馬達持續導通限時保護：滿 FSM_DROGUE_MOTOR_RUN_MS(4.0s) 後強制停止，
            // 轉入純記錄的 APOGEE 狀態（機構此時應已完整展開）
            if (now - ctx->state_entered_ms >= FSM_DROGUE_MOTOR_RUN_MS) {
                ctx->state            = STATE_APOGEE;
                ctx->state_entered_ms = now;
                act.release_drogue    = 1U;   // 停止馬達 (PD13 = LOW)
                act.event             = FSM_EVT_DROGUE_DONE;
            }
            break;

        case STATE_APOGEE:
            // 純狀態記錄：不驅動任何硬體，同一週期立即轉入 DESCENT（僅供遙測/
            // 地面站短暫看到「已達頂點記錄點」，實際部署動作在 DEPLOY_DROGUE 完成）
